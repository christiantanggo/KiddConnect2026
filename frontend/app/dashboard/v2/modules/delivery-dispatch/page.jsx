...
const STATUS_OPTIONS = ['New', 'Contacting', 'Dispatched', 'Assigned', 'PickedUp', 'Completed', 'Failed', 'Cancelled', 'Needs Manual Assist'];
...
const SETTINGS_SUB_TABS = [
  { id: 'approved-numbers', label: 'Approved caller numbers' },
...
export default function DeliveryDispatchPage() {
  ...
  const [newApprovedLabel, setNewApprovedLabel] = useState('');
  const [showAddApprovedNumber, setShowAddApprovedNumber] = useState(false);
  ...
  const handleDeleteRequest = async () => {
    ...
    setDeleteError(null);
    try {
      await deliveryNetworkAPI.deleteRequest(deleteModal.request.id);
  ...
  const handleResetDispatch = async (requestId) => {
    ...
    try {
      await deliveryNetworkAPI.resetDispatch(requestId);
      await load();
    } catch (e) {
  ...
  const handleCallProvider = async (requestId) => {
    ...
    try {
      await deliveryNetworkAPI.retryDispatch(requestId);
      await load();
    } catch (e) {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const [configRes, requestsRes, providersRes, analyticsRes, phoneNumbersRes, dispatchLogRes, savedLocationsRes] = await Promise.all([
        deliveryNetworkAPI.getConfig().catch(() => ({ data: config })),
        deliveryNetworkAPI.getRequests().catch(() => ({ data: { requests: [] } })),
        deliveryNetworkAPI.getApprovedNumbers().catch(() => ({ data: { approved_numbers: [] } })),
        deliveryNetworkAPI.getAnalytics().catch(() => ({ data: {} })),
        deliveryNetworkAPI.getPhoneNumbers().catch(() => ({ data: { phone_numbers: [] } })),
        deliveryNetworkAPI.getDispatchLog().catch(() => ({ data: { log: [] } })),
        deliveryNetworkAPI.getSavedLocations().catch(() => ({ data: { saved_locations: [] } })),
      ]);
      setConfig({
        delivery_phone_numbers: configRes.data?.delivery_phone_numbers ?? [],
        delivery_vapi_assistant_id: configRes.data?.delivery_vapi_assistant_id ?? '',
      });
      setRequests(requestsRes.data?.requests ?? []);
      setApprovedNumbers(providersRes.data?.approved_numbers ?? []);
      setAnalytics(analyticsRes.data ?? null);
      setAvailablePhoneNumbers(phoneNumbersRes.data?.phone_numbers ?? []);
      setDispatchLog(dispatchLogRes.data?.log ?? []);
      setSavedLocations(savedLocationsRes.data?.saved_locations ?? []);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      deliveryNetworkAPI.getDispatchLog(selectedRequestId),
      deliveryNetworkAPI.getRequestActivity(selectedRequestId),
    ])
    ...
    const interval = setInterval(async () => {
      try {
        const [requestsRes, dispatchLogRes] = await Promise.all([
          deliveryNetworkAPI.getRequests(),
          deliveryNetworkAPI.getDispatchLog(),
        ]);
        setRequests(requestsRes.data?.requests ?? []);
        setDispatchLog(dispatchLogRes.data?.log ?? []);
      } catch (_) {}
  ...
  useEffect(() => {
    if (websitePageKey && activeTab === 'settings' && settingsSubTab === 'website-pages') {
      ...
      deliveryNetworkAPI.getWebsitePage(websitePageKey)
        .then((res) => {
  ...
  const saveWebsitePage = async () => {
    ...
    try {
      await deliveryNetworkAPI.updateWebsitePage(websitePageKey, contentToSave);
  ...
  const handleWebsiteHeroUpload = async (pageKey, file) => {
    ...
    try {
      ...
      const res = await deliveryNetworkAPI.uploadWebsiteHero(formData, pageKey);
      ...
      setWebsitePageDirty(false);
      await deliveryNetworkAPI.updateWebsitePage(pageKey, newContent);
  ...
  const saveConfig = async (overrides = null) => {
    try {
      ...
      const toSend = {
        delivery_phone_numbers: Array.isArray(c.delivery_phone_numbers) ? c.delivery_phone_numbers : [],
        delivery_vapi_assistant_id: c.delivery_vapi_assistant_id || null,
      };
      const res = await deliveryNetworkAPI.updateConfig(toSend);
      const data = res.data || {};
      const { link_result, ...configRest } = data;
      setConfig((prev) => ({
        ...prev,
        delivery_phone_numbers: configRest.delivery_phone_numbers ?? c.delivery_phone_numbers ?? [],
        delivery_vapi_assistant_id: configRest.delivery_vapi_assistant_id ?? c.delivery_vapi_assistant_id ?? '',
      }));
      ...
      console.error('[DeliveryDispatch] save config error', e);
  ...
  const linkAgentToNumbers = async () => {
    ...
    setLinkResult(null);
    try {
      const res = await deliveryNetworkAPI.linkAgent();
      const d = res.data || {};
      ...
      if (d.linked?.length > 0 && !d.errors?.length) {
        ...
        setCreateAgentError(d.message || 'No numbers linked. Add delivery phone numbers in Settings and save first.');
      }
      setTimeout(() => setConfigSaveMessage(null), 5000);
      await load();
    } catch (e) {
  ...
  const addPhoneNumber = async () => {
    ...
    const list = [...(config.delivery_phone_numbers || []), withPlus];
    setConfig((c) => ({ ...c, delivery_phone_numbers: list }));
    setNewPhone('');
    await saveConfig({ delivery_phone_numbers: list });
  };

  const addSelectedPhoneNumber = async () => {
    if (!selectedNumberToAdd) return;
    const list = [...(config.delivery_phone_numbers || []), selectedNumberToAdd];
    setConfig((c) => ({ ...c, delivery_phone_numbers: list }));
  ...
  const removePhoneNumber = async (idx) => {
    const list = (config.delivery_phone_numbers || []).filter((_, i) => i !== idx);
    setConfig((c) => ({ ...c, delivery_phone_numbers: list }));
    await saveConfig({ delivery_phone_numbers: list });
  };
  ...
  const createAgent = async () => {
    ...
    try {
      const res = await deliveryNetworkAPI.createAgent();
      const id = res.data?.assistant_id;
      if (id) setConfig((c) => ({ ...c, delivery_vapi_assistant_id: id }));
      setConfigDirty(false);
      await load();
      // Attach the agent to configured emergency phone number(s) in VAPI
      try {
        const linkRes = await deliveryNetworkAPI.linkAgent();
  ...
  const createApprovedNumber = async () => {
    const phone = (newApprovedPhone || '').trim().replace(/\D/g, '');
    if (!phone || phone.length < 10) return;
    try {
      await deliveryNetworkAPI.createApprovedNumber({
        phone_number: phone.length === 10 ? `+1${phone}` : `+${phone}`,
        label: (newApprovedLabel || '').trim() || null,
      });
      setNewApprovedPhone('');
      setNewApprovedLabel('');
      setShowAddApprovedNumber(false);
      load();
    } catch (e) {
      console.error('[DeliveryDispatch] create approved number error', e);
    }
  };

  const saveApprovedNumberEdit = async (id, updates) => {
    try {
      await deliveryNetworkAPI.updateApprovedNumber(id, updates);
      setEditingApprovedId(null);
      load();
    } catch (e) {
      console.error('[DeliveryDispatch] update approved number error', e);
    }
  };

  const deleteApprovedNumberById = async (id) => {
    if (!confirm('Remove this approved number?')) return;
    try {
      await deliveryNetworkAPI.deleteApprovedNumber(id);
      load();
    } catch (e) {
      console.error('[DeliveryDispatch] delete approved number error', e);
    }
  };

  const updateRequestStatus = async (id, status) => {
    try {
      await deliveryNetworkAPI.updateRequest(id, { status });
      load();
      // If the detail modal is open for this request, refetch activity so the new status change appears
      if (selectedRequestId === id) {
        try {
          const res = await deliveryNetworkAPI.getRequestActivity(id);
  ...
  const isDispatched = (r) => ['Dispatched', 'Assigned', 'PickedUp', 'Completed'].includes(r.status);
  const pendingRequests = requests.filter((r) => !isDispatched(r)).slice(0, 100);
  const dispatchedRequests = requests.filter(isDispatched);
  ...
  const derivedAnalytics = {
    ...
    dispatch_rate: requests.length
      ? Math.round(
          (requests.filter((r) => ['Dispatched', 'Assigned', 'PickedUp', 'Completed'].includes(r.status)).length / requests.length) * 100
        )
      : 0,
  };
        ...
        <div className="p-4 md:p-6 max-w-6xl mx-auto">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="ml-auto flex items-center gap-2">
              <button type="button" onClick={createAgent} disabled={creatingAgent} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium rounded-lg border border-slate-200 bg-white hover:bg-slate-50 disabled:opacity-60" title="Create or replace VAPI delivery assistant">
          ...
          <p className="text-slate-600 mb-4">
            Schedule pickups and deliveries via phone, SMS, form, and chat. Public page: <a href="/deliverydispatch" target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">/deliverydispatch</a>
          </p>
          ...
          {activeTab === 'dashboard' && (
            <>
              {approvedNumbers.length === 0 && (config.delivery_phone_numbers || []).length > 0 && (
                <div className="mb-6 p-4 rounded-xl border border-amber-200 bg-amber-50">
                  <h3 className="text-sm font-semibold text-amber-800 mb-1">Add approved caller numbers</h3>
                  <p className="text-sm text-amber-700 mb-2">So calls to the delivery line are attributed to your business. Add at least one in Settings.</p>
              ...
              {/* Stats cards — same layout as emergency dispatch */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                {(config.delivery_phone_numbers || [])[0] && (
                  <div className="p-5 rounded-xl bg-gradient-to-br from-emerald-600 to-emerald-700 text-white shadow-lg">
                    ...
                    <h3 className="text-sm font-semibold opacity-90">Delivery line</h3>
                    <p className="text-lg font-bold mt-1">{(config.delivery_phone_numbers || [])[0]}</p>
                ...
                <div className="p-5 rounded-xl border border-slate-200 bg-white shadow-sm border-l-4 border-l-emerald-500">
                  <h3 className="text-sm font-semibold text-slate-500">Dispatch rate</h3>
                  <p className="text-3xl font-bold text-slate-800 mt-1">{displayAnalytics.dispatch_rate}%</p>
                </div>
              </div>
              {/* Needs escalation — same pattern as emergency */}
              {(() => {
                ...
                return (
                  <div className="mb-8 p-5 rounded-xl border-2 border-amber-300 bg-amber-50">
                    ...
                    <p className="text-sm text-amber-800 mb-4">These deliveries need manual follow-up. A Tavari operator can retry dispatch or assist the customer.</p>
                    <div className="space-y-2">
                      {needsEscalation.map((r) => (
                        <div key={r.id} className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-lg bg-white border border-amber-200">
                          <div>
                            <p className="text-sm font-medium text-slate-800">{r.reference_number || r.callback_phone}</p>
                            <p className="text-xs text-slate-600">{r.callback_phone} · {r.priority || 'Schedule'} · {formatDate(r.created_at)}</p>
                            {r.delivery_address && <p className="text-xs text-slate-500 truncate max-w-xs">{r.delivery_address}</p>}
                          </div>
                          <span className="text-xs font-medium px-2 py-1 rounded bg-amber-200 text-amber-900">Needs manual assist</span>
                        </div>
                      ))}
                    </div>
              ...
              {/* Recent activity — same pattern as emergency */}
              <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-5">
                ...
                <p className="text-slate-600 text-sm mb-4">All delivery requests from phone, SMS, form, and chat in one place.</p>
                {requests.length === 0 ? (
                    ...
                    {[...requests]
                      ...
                      .map((r) => (
                        ...
                        >
                          <div>
                            <p className="text-sm font-medium text-slate-800">{r.reference_number || r.callback_phone}</p>
                            <p className="text-xs text-slate-500">{formatDate(r.created_at)} · {r.intake_channel || 'form'} · {r.priority || '—'}</p>
                          </div>
          ...
          {activeTab === 'settings' && (
            ...
            <section className="bg-white rounded-xl border border-slate-200 p-6">
              ...
              {settingsSubTab === 'approved-numbers' && (
            <section className="bg-white rounded-xl border border-slate-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-medium">Approved caller numbers</h2>
                <button type="button" onClick={() => setShowAddApprovedNumber(true)} className="inline-flex items-center gap-1 px-3 py-2 bg-slate-800 text-white rounded-lg text-sm hover:bg-slate-700">
                  <Plus className="w-4 h-4" /> Add number
                </button>
              </div>
              <p className="text-slate-600 text-sm mb-4">When a caller reaches the delivery line from one of these numbers, requests are attributed to this business.</p>
              {showAddApprovedNumber && (
                <div className="mb-4 p-4 bg-slate-50 rounded-lg space-y-2">
                  <input placeholder="Phone number (e.g. +15551234567)" className="w-full rounded border px-3 py-2 text-sm" value={newApprovedPhone} onChange={(e) => setNewApprovedPhone(e.target.value)} />
                  <input placeholder="Label (optional)" className="w-full rounded border px-3 py-2 text-sm" value={newApprovedLabel} onChange={(e) => setNewApprovedLabel(e.target.value)} />
                  <div className="flex gap-2">
                    <button type="button" onClick={createApprovedNumber} className="px-4 py-2 bg-emerald-600 text-white rounded text-sm">Add</button>
                    <button type="button" onClick={() => { setShowAddApprovedNumber(false); setNewApprovedPhone(''); setNewApprovedLabel(''); }} className="px-4 py-2 border rounded text-sm">Cancel</button>
                  </div>
                </div>
              )}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {approvedNumbers.length === 0 ? (
                      <tr><td colSpan={3} className="py-4 text-slate-500 text-center">No approved numbers. Add caller numbers to attribute delivery requests to this business.</td></tr>
                    ) : (
                      approvedNumbers.map((a) => (
                        <tr key={a.id} className="border-b border-slate-100">
                          <td className="py-2 pr-2">{a.phone_number}</td>
                          <td className="py-2 pr-2">{a.label || '—'}</td>
                          <td className="py-2">
                            <button type="button" onClick={() => deleteApprovedNumberById(a.id)} className="text-red-600 hover:underline text-xs p-1 rounded hover:bg-red-50" title="Remove"><Trash2 className="w-4 h-4 inline" /></button>
                          </td>
                        </tr>
                      ))
                    )}
              ...
              {settingsSubTab === 'saved-locations' && (
            <section className="bg-white rounded-xl border border-slate-200 p-6">
              <h2 className="text-lg font-medium mb-2">Saved locations</h2>
              <p className="text-slate-600 text-sm mb-4">Default pickup, named pickups, and frequent delivery addresses for faster intake.</p>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200 text-left text-slate-600">
                  ...
                  <tbody>
                    {savedLocations.length === 0 ? (
                      <tr><td colSpan={4} className="py-4 text-slate-500 text-center">No saved locations. Add default pickup or frequent addresses.</td></tr>
                    ) : (
                      savedLocations.map((loc) => (
                        <tr key={loc.id} className="border-b border-slate-100">
                          <td className="py-2 pr-2">{loc.type}</td>
                          <td className="py-2 pr-2">{loc.name || '—'}</td>
                          <td className="py-2 pr-2 max-w-[200px] truncate" title={loc.address}>{loc.address || '—'}</td>
                          <td className="py-2">
                            <button type="button" onClick={async () => { try { await deliveryNetworkAPI.deleteSavedLocation(loc.id); load(); } catch (e) { console.error(e); } }} className="text-red-600 hover:underline text-xs">Remove</button>
                          </td>
                        </tr>
                      ))
                    )}
              ...
              <p className="text-slate-500 text-xs mt-2">Add via API or future form: POST /v2/delivery-network/saved-locations with type (default_pickup | named_pickup | frequent_delivery), name, address, contact.</p>
            </section>
          )}

              {settingsSubTab === 'partner-api' && (
            <section className="bg-white rounded-xl border border-slate-200 p-6">
              <h2 className="text-lg font-medium mb-2">Partner API</h2>
              <p className="text-slate-600 text-sm">External API for partner applications to create or query delivery requests.</p>
              <div className="mt-4 p-4 rounded-lg bg-amber-50 border border-amber-200 text-amber-800">
                <p className="font-medium">Coming soon</p>
                <p className="text-sm mt-1">Partner API will be available after MVP. You will be able to create deliveries and check status via API with authentication.</p>
              </div>
            </section>
          )}
            ...
            <section className="bg-white rounded-xl border border-slate-200 p-6">
              ...
              <div className="space-y-4">
                ...
                {config.delivery_vapi_assistant_id && (config.delivery_phone_numbers || []).length > 0 && (
                  <div className="p-4 rounded-xl border border-slate-200 bg-slate-50">
                    <p className="text-sm font-medium text-slate-800 mb-2">Agent ↔ phone number connection</p>
                    <p className="text-sm text-slate-600 mb-3">For calls to the delivery line to reach the AI agent, the agent must be linked to each number in VAPI. Save config or use the button below to link.</p>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {(config.delivery_phone_numbers || []).map((n, i) => (
                  ...
                  <div className="flex gap-2 mb-2">
                    ...
                    >
                      ...
                      {(availablePhoneNumbers || []).filter((pn) => !(config.delivery_phone_numbers || []).includes(pn.e164 || pn.number)).map((pn) => {
                ...
                <table className="w-full text-sm border-separate" style={{ borderSpacing: '0 6px' }}>
                  <thead>
                    <tr className="text-left text-slate-600">
                      ...
                      <th className="pb-2 pr-2">Callback</th>
                      <th className="pb-2 pr-2 max-w-[140px]">Delivery address</th>
                  ...
                  <tbody>
                    {(communicationSubTab === 'pending' ? pendingRequests : dispatchedRequests).length === 0 ? (
                      <tr><td colSpan={8} className="py-4 text-slate-500 text-center">{communicationSubTab === 'pending' ? 'No pending requests' : 'No dispatched requests yet'}</td></tr>
                    ) : (
                      (communicationSubTab === 'pending' ? pendingRequests : dispatchedRequests).map((r) => {
                        const rowBg = isDispatched(r)
                        ...
                        const channelLabel = r.intake_channel || 'form';
                        ...
                        return (
                          <tr
                            ...
                            onClick={() => setSelectedRequestId(r.id)}
                          >
                            <td className="py-2 pr-2 font-mono text-slate-800">{r.reference_number || '—'}</td>
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-slate-200 text-left text-slate-600">
                          ...
                          <th className="pb-1 pr-2">Broker</th>
                      ...
                      <tbody>
                        {dispatchLog.slice(0, 30).map((entry) => (
                          <tr key={entry.id} className="border-b border-slate-100">
                            <td className="py-1 pr-2 font-mono text-slate-500">{String(entry.delivery_request_id || entry.id).slice(0, 8)}…</td>
                            <td className="py-1 pr-2">{entry.broker_id || '—'}</td>
          ...
          {activeTab === 'settings' && (
            <section className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-4">
                <p className="text-sm text-slate-500">Dispatch rate</p>
                <p className="text-2xl font-semibold">{displayAnalytics.dispatch_rate}%</p>
              </div>
            </section>
          )}
                    ...
                    {req ? (
                      <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                          <p><span className="text-slate-500">Reference</span> {req.reference_number || '—'}</p>
                          <p><span className="text-slate-500">Callback</span> {req.callback_phone}</p>
                          <p><span className="text-slate-500">Priority</span> {req.priority || '—'}</p>
                          ...
                          {req.delivery_address && <p className="sm:col-span-2"><span className="text-slate-500">Delivery address</span> {req.delivery_address}</p>}
                          {req.pickup_address && <p className="sm:col-span-2"><span className="text-slate-500">Pickup</span> {req.pickup_address}</p>}
                          {req.recipient_name && <p><span className="text-slate-500">Recipient</span> {req.recipient_name}</p>}
                          {req.package_description && <p className="sm:col-span-2"><span className="text-slate-500">Package</span> {req.package_description}</p>}
                        </div>
                        <div>
                          ...
                          {loadingDetail ? (
                            ...
                            <p className="text-slate-500">No broker attempts yet.</p>
                          ) : (
