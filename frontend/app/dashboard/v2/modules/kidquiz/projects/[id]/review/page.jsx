...
export default function ReviewPage() {
  ...
  if (loading) return <AuthGuard><V2AppShell><div className="text-center py-20" style={{ color: 'var(--color-text-muted)' }}>Loadingâ€¦</div></V2AppShell></AuthGuard>;
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px' }}>
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-2xl p-5" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
              <h2 className="font-bold text-base mb-3" style={{ color: 'var(--color-text-main)' }}>ðŸ“‹ Quiz Preview</h2>
              <div className="rounded-xl p-4 mb-3" style={{ background: '#1e1e2e' }}>
                ...
                {answers.map(a => (
                  <div key={a.label} className="flex items-center gap-2 mb-1.5">
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${a.is_correct ? 'bg-green-500 text-white' : 'bg-gray-600 text-gray-200'}`}>{a.is_correct ? 'âœ“' : a.label}</span>
            ...
            <div className="rounded-2xl p-5" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-bold text-base" style={{ color: 'var(--color-text-main)' }}>ðŸ¤– YouTube Metadata</h2>
                <button onClick={generateMeta} disabled={generatingMeta} className="text-xs px-3 py-1.5 rounded-lg font-semibold text-white" style={{ background: generatingMeta ? '#9ca3af' : '#6366f1' }}>
                  {generatingMeta ? 'Generatingâ€¦' : 'âœ¨ AI Generate'}
                </button>
              </div>
            ...
            <button onClick={saveAndApprove} disabled={approving} className="w-full py-4 rounded-xl font-bold text-white text-lg" style={{ background: 'linear-gradient(135deg, #6366f1, #ec4899)', opacity: approving ? 0.7 : 1 }}>
              {approving ? 'Approvingâ€¦' : 'âœ… Approve & Render Video â†’'}
            </button>
          </div>
        </div>
      </V2AppShell>
