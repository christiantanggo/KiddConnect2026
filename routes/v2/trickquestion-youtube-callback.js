/**
 * Trick Question (per-channel) YouTube OAuth callback — PUBLIC route, no auth.
 * Used when a channel has a custom OAuth app and is the trick question channel.
 * Redirect URI in Google must be https://api.tavarios.com/api/v2/trickquestion/youtube/callback
...
/** Single source of truth for trick-question channel OAuth redirect URI. Used by auth-url and by this callback. */
export function getTrickQuestionYoutubeRedirectUri() {
  if (process.env.NODE_ENV === 'production' && (process.env.YOUTUBE_REDIRECT_URI || '').includes('api.tavarios.com')) {
    return 'https://api.tavarios.com/api/v2/trickquestion/youtube/callback';
  }
  ...
  return `${base}/api/v2/trickquestion/youtube/callback`;
}

function getRedirectUri() {
  return getTrickQuestionYoutubeRedirectUri();
}
...
router.get('/youtube/callback', async (req, res) => {
  try {
    ...
    console.log('[Trick Question YouTube Callback] received query:', JSON.stringify({ hasCode: !!code, hasState: !!state, error: error || null, queryKeys: Object.keys(req.query || {}) }));

    const { redirectBase } = parseState(state || '');

    if (error) {
      console.log('[Trick Question YouTube Callback] Google returned error param:', error, 'description:', req.query.error_description || '');
    ...
    if (!code) {
      console.warn('[Trick Question YouTube Callback] No code in request — cannot exchange for tokens. Full URL path+query:', req.url);
    ...
    try {
      ...
      console.error('[Trick Question YouTube Callback] getToken failed:', data?.error, data?.error_description, 'redirect_uri=', redirectUri);
    ...
    console.log('[Trick Question YouTube Callback] Saved credentials businessId=', businessId, 'orbixChannelId=', orbixChannelId, 'usage=', usageManual ? 'manual' : 'auto', 'youtube_channel_id=', channel.id);
    ...
    console.error('[Trick Question YouTube Callback] Error:', err?.message);
    if (isInvalidGrant) {
      return res.redirect(`${redirectBase}/dashboard/v2/modules/orbix-network/settings?error=invalid_grant`);
    }
